...
% FIR but in FPGA will be differnet because will be decimasion
if isempty(Hd1)
...
fc = 200000; % Middle Frequency of FM FLo   
dt = 1/fs; % seconds per sample 
Cycles_T = 120; % seconds 
A = 1;
fDev = 25000;
T = (1/fc)*Cycles_T;
tt = 0:dt:T+dt;
...
% NCO X RF --------------------------------------
NCO = A*sin(2*pi*(fc+fDev*2)*tt);% DEV*200 = Because need for slop FIR to be in middle
x = A*sin(2*pi*8000*tt);
...
DC_LOW = filter(Hd3,data);

nexttile;
plot(tt,DC_LOW);
title('DATA WITHOUT DC');
xlabel('SAMPLES')
ylabel('Amplitude')

nexttile;
F_out3 = cast(DC_LOW,"double")
F_out4 = fftshift(abs(fft(F_out3))./length(F_out3));   
plot(tt,log(F_out4)) ;
title('DATA OUT FFT 4');
xlabel('Frequency');
ylabel('Amplitude');
